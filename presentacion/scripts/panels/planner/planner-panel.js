...
export function createPlannerPanel({ panelId = "plannerPanel" } = {}) {
  ...
  function applyRecalculation(plannerResult = {}) {
    if (!plannerResult || typeof plannerResult !== "object") return;

    if (plannerResult.mode === "basic") {
      state.mode = "basic";
      state.itinerary_a = plannerResult.itinerary_a ?? state.itinerary_a;
      state.itinerary_b = plannerResult.itinerary_b ?? state.itinerary_b;
      state.routes = null;
    } else if (plannerResult.mode === "route") {
      state.mode = "route";
      state.routes = plannerResult.routes ?? state.routes;
      state.itinerary_a = null;
      state.itinerary_b = null;
    }

    state.lastRequest = plannerResult.request ?? state.lastRequest;
    renderResults(resultsList, resultsSection, _onHighlightRoute);
  }

  render(panel, submitBtn);

  return { setAvailability, onHighlightRoute, applyRecalculation };
}
